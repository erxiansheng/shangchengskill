...
2. **独立** 的 Vue 3 后台控制台（入口 `admin/index.html`，路由 `/admin/*`，
   ⛔ **不是** 主应用里的 tab，必须是 Vite 多入口拆出来的独立子应用）
...
| 后台鉴权 | **独立** JWT scope（`scope:"admin"`）+ `X-Admin-Password` 二次头 |
...
加入 `admin:list`），其登录密码同时可用于登录 `/admin` 后台。
Agent 完工时要明确告诉用户：「第一个打开站点完成注册的人就是站长，
建议立刻自己抢注」。`admin_panel_password` 仅作为 `/admin/*` 的二次头。
...
### 后台（Vue，入口 `/admin/`，⛔ 必须独立 HTML 入口）

| 路由 | 用途 |
|---|---|
| `/admin/login` | 独立登录页（不复用用户端 token） |
| `/admin/dashboard` | 实时统计（用户/订单/商品/收入） |
| `/admin/users` | 用户列表 + 角色 + 封禁 |
| `/admin/products` | 商品 CRUD + 双支付模式 |
| `/admin/categories` | 11 默认分类增删改、图标、排序 |
| `/admin/audit` | 待审核商品 |
...
| 层级 | 凭证 | TTL / 携带方式 |
|---|---|---|
| 用户端 | scope=`user` JWT | TTL 7 天，`Authorization: Bearer …` |
| 后台 | scope=`admin` JWT + `X-Admin-Password` 二次头 | TTL 8 小时，独立 secret |
| Edge ↔ Cloud 互调 | `INTERNAL_KEY` + `JWT_SECRET` | 两端 inline 写死，⛔ 不入 git |

✅ 密码哈希：bcrypt cost 12  
✅ **第一个 `/auth/register` 成功的账号自动 `role=admin` + 入 `admin:list`**  
...
#       JWT_SECRET / INTERNAL_KEY / ADMIN_INIT_USERNAME / ADMIN_INIT_PASSWORD
