...
#    「全球可用区（不含中国大陆）」；选错了只能删项目重建。

# 4. Agent 引导你到控制台：
#    a. 创建 KV 命名空间 mall_kv → 项目「环境变量与绑定」绑定为 MY_KV
#    b. 二选一让 API 跨过 token 鉴权：
#       A) 加速区域选「全球（不含中国大陆）」→ 境外访问临时域名免 token
#       B) 绑定一个自定义域名 → 全球可用、需 ICP 备案（国内站）或免备案（国际站）
...
|---|---|---|---|---|
| `china + 自定义域名` | <https://console.cloud.tencent.com/edgeone/pages> | 中国 + 全球 | 必须，需 ICP 备案 | 国内正式上线 |
| `global + 自定义域名` | <https://console.tencentcloud.com/edgeone/pages> | 全球 | 必须，免备案 | 出海 / 海外业务 |
| `china + 不含中国大陆` | 同上 | 仅境外 | 不需要 | 国内账号但服务境外 |
| `global + 不含中国大陆` | 同上 | 仅境外 | 不需要 | 海外个人 demo / Skill 演示 |

🚨 **加速区域在项目创建后不可修改**！如果选择了「不含中国大陆」路径，
必须在控制台创建项目时就选「全球可用区（不含中国大陆）」，否则只能删项目重建。
...
| 现象 | 可能原因 | 处理 |
|---|---|---|
| `/api/v1/*` 全 401 / 544 | preset 临时域名 token 拦了 Cloud→Edge 内部回调 | 切「全球可用区(不含中国大陆)」从境外访问，或绑自定义域名 |
