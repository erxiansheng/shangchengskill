...
5. **admin_panel_password**（**后台二次验证密码**，调用任何 `/admin/*` API 时
   必须携带 `X-Admin-Password` 头；与上面登录密码**不同**；未填则后台 API 全部 503,默认`admin123456`）
6. **target_domain**（部署后要绑定的自定义域名，例如 `mall.example.com`；可留空但不推荐）。
   当 `target_domain` 留空时，项目只能走临时域名，Agent 会在 4.2.b 提示用户到控制台把加速区域改为
   “全球可用区（不含中国大陆）”，否则 API 会被 token 鉴权拦截。
7. **site_region**（`china 绑定自定义域名` | `global 绑定自定义域名`| `china 选“不含中国大陆”区域`| `global 选“不含中国大陆”区域`，默认 `china 绑定自定义域名`）。决定 `edgeone login --site` 参数。
   - `china`：控制台 <https://console.cloud.tencent.com/edgeone/pages>；绑定自定义域名需 ICP 备案，免备案可选“不含中国大陆”区域 国内不可访问。
   - `global`：控制台 <https://console.tencentcloud.com/edgeone/pages>；绑定自定义域名需 ICP 备案，免备案可选“不含中国大陆”区域 国内不可访问。
   - 🚨 **加速区域一旦项目创建后不可修改**。Agent 必须在本步驤请用户明确选定 `site_region` 后才能进入 4.1；
     如果选“选不含中国大陆”区域→ 4.1 创建后的 preset 临时域名从境外即可免 token 联调，无须自定义域名，4.2.b 选方案 A；
     如果选“绑定自定义域名”→ 4.2.b 选方案 B，项目默认全球加速区域不变。
     **选错唯一补救是在控制台删掉项目重建**，部署后才发现不能反悔。
8. **全部默认**（可以选择使用全部默认值,但一定要把每项默认值告诉用户是什么）

> 💡 **首次注册账号 = 管理员**：本商城后端会把 **第一个通过 `/auth/register` 注册成功的用户**
> 自动设为管理员（`role=admin` + 加入 `admin:list`），其登录密码同时可用于登录 `/admin` 后台。
> 不再需要单独维护 `default_admin_*` 与 `admin_panel_password`：上述字段仅用于完全无人注册时
> 的 fallback 兜底；`admin_panel_password` 仍作为 `/admin/*` 的二次验证头 `X-Admin-Password`。
...
> ⚠️ **全栈商城（Edge ↔ Cloud 互调）必须绑定自定义域名（见 4.2.b）**。
...
> 因此：**不要在 preset 临时域名上验证业务逻辑**，必须先绑一个自有域名，
> 所有验证 / 首次 seed 均在该域名上进行。
>
> ⚠️ **站点选择由用户决定**。`site_region` 参数控制 `edgeone login --site` 值；
> 国内站需 ICP 备案才能绑自定义域名；国际站免备案。两个站点都需要在 4.2.b 处理 token 鉴权问题。
>
> 🚨 **项目创建后加速区域不可变更**：如果用户选了“选不含中国大陆”路径（方案 A），
> Agent 必须在调 4.1 的 `edgeone pages deploy -n` 前先提醒用户：控制台创建项目时就选“全球可用区（不含中国大陆）”，否则
> 默认创建后只能删掉重建。如果选了“绑定自定义域名”路径（方案 B），默认全球可用区即可，后续 4.2.b 绑域名。
...
edgeone pages deploy -n <project-name>        # 4) 首次部署，自动创建 edgeone.json
```
...
#### 4.2.b 选 A 或 B：让 API 能跨过 token 鉴权（**二选一，不可跳过**）

> 勿直接访问项目 preset 临时域名 `*.edgeone.cool` 跳 4.4 验证；default token 策略会让
> Cloud↔Edge 内部回调被 401 拦截，所有 KV 写失败。

**方案 A — 初次部署时就选“全球可用区（不含中国大陆）”（免域名、仅限境外访问）**

> 🚨 **项目加速区域一旦创建后不可修改**！必须在进入 4.1 之前就确认用户选了“不含中国大陆”路径；
> 如果刚才 4.1 默认创建了全球区域项目但用户现在需要 A，唯一办法是**在控制台删掉该项目重新 `edgeone pages deploy -n <新名>`**。

1. EdgeOne Pages 控制台 → 项目设置 → **加速区域** → 创建项目时就选 **“全球可用区（不含中国大陆）”** 保存。
2. 项目临时域名 `https://<project>.edgeone.cool` 从境外网络访问将**免 token**直接联通，Agent 应**直接把该 preset 域名作为 `<DEPLOY_URL>` 交付用户**（后续 4.4 / 4.5 验证全部走该 URL）。
3. 中国大陆境内访问仍返 401。**适合**：Skill 开发者 / 在境外服务的全球用户 / 需要快速验证 demo 不想买域名。

**方案 B — 绑定自定义域名（全球可用、需一个可用域名）**

1. EdgeOne Pages 控制台 → 项目 → **域名 → 添加自定义域名**，输入 `mall.example.com`。
2. 控制台会给出一个 CNAME 目标（如 `*.edgeonedy1.com`）。到你的域名注册商加一条 CNAME。
3. 返回 EdgeOne 控制台 点 **校验**，等状态变 “生效”。同页点 **申请免费 SSL DV 证书**。
4. 拿到一个永久、不带 token 的 URL，记为 `<DEPLOY_URL>`，后续验证都走该域名。

> ⚠️ **国内站** 需 ICP 备案才能绑域名；未备案请选方案 A，或切到国际站使用免备案域名。
>
> ⚠️ 用户手上没有可用域名：推荐选方案 A；或到 Cloudflare Registrar / Namecheap / 腾讯云国际站
> 注册一个年费几块钱的 `.xyz` / `.online`。**不要**两者都未完成就进 4.4。
